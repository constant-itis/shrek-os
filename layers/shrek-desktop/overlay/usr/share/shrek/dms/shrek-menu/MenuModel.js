...
//
// Only two things differ from upstream, both in the guard batch (everything else is untouched):
//   1. GUARD_READERS is empty — Shrek ships no memoized omarchy-* reader helpers. The substitution
//      machinery below is kept intact so shrek-* readers can be added by appending to this array.
//   2. guardHelpers() is rewritten for a sealed Debian image (dpkg + command + systemd unit presence)
//      instead of Arch pacman. Nothing installs post-boot, so "package present" is re-derived to
//      command / unit / dpkg-state present — see the function for the exact helper names menu.jsonc
//      may call (shrek-pkg-present, shrek-cmd-present, shrek-unit-active, ...).
...
// Shrek ships no memoized reader helpers yet (Omarchy had omarchy-default-browser etc.). Left empty
// on purpose: guardPrelude/substituteGuardReaders below iterate this list, so an empty array is a
// no-op and every `$(...)` in a guard just runs its real command. To add a fast-path reader later,
// append its exact command name here — nothing else needs to change.
var GUARD_READERS = []

// Package, command and unit presence account for most of what the guards ask,
// and asked one at a time they are almost all fork: the shipped menu spends
// over a second on them. Answer them inside the guard process instead. These
// helpers shadow nothing real, so menu.jsonc calls them by name; they have to
// agree with themselves everywhere, including for no arguments at all (present
// is true of nothing, missing is not).
//
// Shrek is a SEALED Debian image: nothing installs post-boot, so "installed"
// is a fixed fact, and dpkg's own database is the source of truth. One
// `dpkg-query -W` load fills an associative array once; every shrek-pkg-*
// lookup after that is a hash hit, not a fork -- same optimization Omarchy got
// from `pacman -Qq`, just Debian-native and without pacman's COLUMNS-wrapping
// parse hazard (`-W -f` is one machine-readable line per package).
//
// dpkg resolves a name through what installed packages Provide, not just what
// they are called (with gvim installed, `vim` is provided), so the set carries
// Provides too -- else a guard offers to "install" what is already there.
// Version constraints are stripped from Provides entries (`foo (>= 1)` -> foo);
// no shipped guard writes a constrained name, so a bare-name set answers all.
function guardHelpers() {
  return 'declare -A __shrek_pkgs=()\n'
    + 'while IFS="|" read -r __shrek_name __shrek_status __shrek_provides; do\n'
    + '  [[ $__shrek_status == "install ok installed" ]] || continue\n'
    + '  __shrek_pkgs[$__shrek_name]=1\n'
    + '  if [[ -n $__shrek_provides ]]; then\n'
    + '    IFS="," read -ra __shrek_prov <<< "$__shrek_provides"\n'
    + '    for __shrek_p in "${__shrek_prov[@]}"; do\n'
    + '      __shrek_p=${__shrek_p%%(*}; __shrek_p=${__shrek_p// /};\n'
    + '      [[ -n $__shrek_p ]] && __shrek_pkgs[$__shrek_p]=1;\n'
    + '    done\n'
    + '  fi\n'
    + 'done < <(dpkg-query -W -f=\'${binary:Package}|${Status}|${Provides}\\n\' 2>/dev/null)\n'
    + '__shrek_pkg_has() { [[ -n ${__shrek_pkgs[$1]-} ]]; }\n'
    + 'shrek-pkg-present() { local p; for p in "$@"; do __shrek_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'shrek-pkg-missing() { local p; for p in "$@"; do __shrek_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'shrek-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'shrek-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
    + 'shrek-unit-active() { local u; for u in "$@"; do systemctl is-active --quiet "$u" || return 1; done; return 0; }\n'
    + 'shrek-unit-enabled() { local u; for u in "$@"; do systemctl is-enabled --quiet "$u" 2>/dev/null || return 1; done; return 0; }\n'
}
