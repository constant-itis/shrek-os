...
// SHREK-OS OVERLAY — vendored patch of DankMaterialShell's Services/WeatherService.qml.
// SOURCE: DMS v1.6.0 (package dms=1.6.0db1, layers/shrek-desktop/mkosi.conf). This file REPLACES the
// packaged copy at /usr/share/quickshell/dms/Services/WeatherService.qml via the shrek-desktop ExtraTrees
// overlay; the packaged `dms` loads it because sway launches `dms run -c /usr/share/quickshell/dms`.
// The ONLY change vs upstream is `maxRetryAttempts` 3 -> 10 (ADR-009 v2 boot-lag plan B): on the sealed
// appliance the egress warm-up (clock sync -> DoT -> pin -> /etc/hosts) can land the first successful
// weather fetch anywhere in a ~90-150s+ window. Stock DMS makes only 3 fast 30s retries (t=30/60/90) then
// falls into a 60s->120s->240s exponential backoff, so a readiness that lands at ~t=95s is not picked up
// until ~t=150s ("weather looks broken on boot"). Raising the fast-retry count keeps the 30s cadence for
// ~5 min (10 x retryDelay), so readiness is caught within 30s across the whole warm-up; only after 5 min
// of continuous failure does the (unchanged) persistent backoff take over for the genuinely-offline case.
// retryDelay is left at 30000 ON PURPOSE — it equals minFetchInterval, so every retry clears the
// fetchWeather throttle (a shorter delay would be silently dropped). handleWeatherSuccess still resets to
// the 15-min updateInterval once weather lands, so steady-state behavior is identical to upstream.
// ⚠ DRIFT: pinned to DMS v1.6.0. If layers/shrek-desktop/mkosi.conf bumps the dms package, re-diff this
// file against the new upstream Services/WeatherService.qml and re-apply the one change;
// scripts/desktop-weather-cadence-proof.sh asserts the pin still reads 1.6.0.
...
Singleton {
    ...
    property int maxRetryAttempts: 10 // SHREK-OS: was 3 upstream (ADR-009 v2 boot-lag plan B; see header)
