...
| SECOND subscription provider (Codex) — broker/provider pattern generalized; coder UNCHANGED; agentic CLI confined under `bwrap` + file-reader tools disabled so the model cannot read the ro-bound credential (P6-6) | [`phase6-slice6-codex-cli-broker.md`](phase6-slice6-codex-cli-broker.md) | [`crates/codex-broker/src/main.rs`](../crates/codex-broker/src/main.rs) `build_confined_argv`/`invoke_codex`/`cmd_login`/`cmd_health`, [`crates/shrek-policy/src/egress.rs`](../crates/shrek-policy/src/egress.rs) `model-codex-cli` | [`scripts/p6-codex-cli-broker-proof.sh`](../scripts/p6-codex-cli-broker-proof.sh) (request-capture zero-reader proof; broker-side, oracle is the gate) |
| Cross-provider stateful sessions — opaque `X-Shrek-Session` handle → native `--resume`/`exec resume`; broker forwards only the new tail turn; codex session state in a broker-owned per-session dir bound rw into the sterile view (P6-7) | [`phase6-slice7-cross-provider-session.md`](phase6-slice7-cross-provider-session.md) | [`crates/claude-broker/src/main.rs`](../crates/claude-broker/src/main.rs) `Registry`/`run_session_turn`, [`crates/codex-broker/src/main.rs`](../crates/codex-broker/src/main.rs) `Registry`/`run_session_turn`/`CodexSess`, [`crates/coder/src/main.rs`](../crates/coder/src/main.rs) `mint_session_handle` | [`scripts/p6-session-broker-proof.sh`](../scripts/p6-session-broker-proof.sh) (broker-side; oracle is the gate), [`mount-plane-gate`](../image/overlay/usr/lib/shrek/mount-plane-gate) (P62-slice7 sealed coder carries the emit) |
