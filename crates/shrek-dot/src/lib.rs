//! shrek-dot — the sealed DNS-over-TLS resolver, shared by egressd and gatekeeperd.
//!
//! A daemon must sometimes turn a name into IPs to pin, but it must NOT ask `resolved`, NetworkManager,
//! `resolv.conf`, or `getaddrinfo` — all of which uid 1000 can steer (the #3121 lesson: don't use the
//! owner-controlled resolver as a security ORACLE). So it resolves over its OWN transport-authenticated
//! channel:
//!
...
//! records ⇒ `Err`. uid 1000 holds authority over none of the three gates (the roots, the IPs, the
//! expected hostname), so it cannot steer the answer.
//!
//! History: extracted verbatim from `egressd::dot` (ADR-007 S2c) into this shared crate for ADR-008 S4,
//! so gatekeeperd resolves its PRIVILEGED public-egress-profile pins (github/debian/pypi/crates) here
//! too — off `resolved` entirely — while NM/resolved stay fully functional for ordinary USER resolution.
//! egressd keeps the desktop-profile→`Pin` wrappers on top of this primitive.
//!
...
//! check here could not pass (the DoT↔clock cycle break, ADR-007 §5).
...
/// build via `SHREK_DOT_CA` (or the legacy `SHREK_EGRESS_DOT_CA`, kept so ADR-007's desktop-egress
/// proofs still redirect it); the sealed image compiles the override out and always uses the baked path.
pub fn ca_path() -> PathBuf {
    #[cfg(feature = "oracle-env")]
    for var in ["SHREK_DOT_CA", "SHREK_EGRESS_DOT_CA"] {
...
/// Why a DoT resolution did not yield an answer. Every variant is fail-closed at the call site.
#[derive(Debug)]
pub enum DotError {
...
/// As [`resolve_over_dot`], but also reports WHICH sealed resolver answered — the caller journals it.
/// Result addrs are sorted + de-duplicated.
...
mod tests {
    ...
    fn valid_dns_name_rules() {
        ...
        assert!(valid_dns_name("github.com"));
    ...
    fn build_query_shape() {
        ...
        assert_eq!(&q[0..2], &[0x12, 0x34]); // header id
        assert_eq!(&q[2..4], &[0x01, 0x00]); // RD
        assert_eq!(&q[4..6], &[0x00, 0x01]); // QDCOUNT=1
        assert_eq!(&q[12..], &[1, b'a', 2, b'b', b'c', 0, 0, 1, 0, 1]); // qname + QTYPE/QCLASS
    }
    ...
    fn parse_two_a_records_with_compression() {
        ...
        m.extend_from_slice(&0u16.to_be_bytes());
        m.extend_from_slice(&[1, b'a', 1, b'b', 0]); // question "a.b" (name at offset 12)
