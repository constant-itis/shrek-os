//! apply — the ELEMENT-ONLY nft applier for the baked desktop egress table (ADR-007 S2b; generalized
//! for the ADR-009 capability layer, S2).
//!
...
//!   * PINNED capabilities (weather + any catalog capability + the raw tier): the supervisor resolves
//!     each destination's IPv4(s) — the sealed name over the S2c DoT client, a literal verbatim — and
//!     this applier reconciles the ONE generalized `@cap_pinned` concat set (`ipv4 . proto . port`) to
//!     exactly the desired UNION of `(ip, proto, port)` tuples across ALL grants (ADR-009 §4.5). One
//!     baked set + one baked rule serve every capability, so a new capability needs ZERO nft change.
//!     Reconcile diffs against the LIVE set (the real kernel truth); on any nft error it ROLLS BACK the
//!     elements it added this call, so a partial apply never half-opens an allow. The set is computed as
//!     a WHOLE (MF-5) — never a per-entry delete — so removing one grant can never drop another's tuple.
...
/// The ONE generalized capability-pin set (ADR-009 §4.5). Type `ipv4_addr . inet_proto . inet_service`,
/// so ONE set encodes per-element (ip, proto, port) with ZERO per-capability runtime rules — the
/// element-only invariant `[R2-MF-B]` holds for every capability and the raw tier alike. The single
/// baked rule `ip daddr . meta l4proto . th dport @cap_pinned accept` matches it. Generalizes (and
/// renames) the ADR-007 S4 `@raw_pinned` set; the plain-ipv4 `@weather_pinned` set it replaced is gone.
pub const CAP_SET: &str = "cap_pinned";
...
/// One concatenated `@cap_pinned` element: `{ <ip> . <proto> . <port> }`. nft joins argv with spaces,
/// so the `.` concat separators are their own tokens. Idempotent; one element per command. NEVER a rule.
pub fn add_cap_element(addr: Ipv4Addr, proto: Proto, port: u16) -> NftCmd {
    cap_element_cmd("add", addr, proto, port)
}

/// `nft delete element inet shrek_desktop_egress cap_pinned { <ip> . <proto> . <port> }`.
pub fn del_cap_element(addr: Ipv4Addr, proto: Proto, port: u16) -> NftCmd {
    cap_element_cmd("delete", addr, proto, port)
}

fn cap_element_cmd(op: &str, addr: Ipv4Addr, proto: Proto, port: u16) -> NftCmd {
...
/// Parse the concatenated `(ip, proto, port)` elements out of `nft list set … cap_pinned` output. nft
...
pub fn parse_cap_set_elements(listing: &str) -> BTreeSet<(Ipv4Addr, Proto, u16)> {
...
/// Why an apply did not (fully) take effect. The caller maps this to a [`store::FaultKind`] and parks a
/// fault — NO element is left half-applied on the add path (rolled back).
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum ApplyError {
...
/// Reconcile the generalized `@cap_pinned` set to EXACTLY `desired` (the UNION over ALL grants —
/// capabilities + raw — of their resolved `(ip, proto, port)` tuples; ADR-009 §4.5 / MF-5: computed as a
/// whole set, so removing one grant can never drop another's element when the two share a tuple).
/// Element-only, diffing against the LIVE set, with add-rollback on any nft error, so a bless can never
/// half-open. Returns the tuples present after the call (for the caller's log / projection).
pub fn apply_cap(
    ...
    let live = parse_cap_set_elements(&exec.run(&list_set(CAP_SET)).map_err(ApplyError::Nft)?);
    ...
    for &(ip, proto, port) in &to_add {
        if let Err(e) = exec.run(&add_cap_element(ip, proto, port)) {
            for &(a, p, pt) in &added {
                let _ = exec.run(&del_cap_element(a, p, pt)); // best-effort rollback to prior state
            }
            return Err(ApplyError::Nft(format!("add cap {ip}.{}.{port}: {e}", proto.label())));
        }
        added.push((ip, proto, port));
    }
    // Then delete the stale tuples. A delete failure leaves a (bounded, previously-blessed) stale allow;
    // report it but do NOT roll back the adds (those are the desired, correct state).
    for &(ip, proto, port) in &to_del {
        if let Err(e) = exec.run(&del_cap_element(ip, proto, port)) {
            return Err(ApplyError::Nft(format!("delete cap {ip}.{}.{port}: {e}", proto.label())));
        }
    }
...
mod tests {
    ...
    /// Recording executor: canned `list set`/`list chain` replies, records every command, and can be
    /// scripted to fail the Nth mutating command (to exercise rollback).
    struct Rec {
    ...
    #[test]
    fn cap_element_cmd_is_a_concat_add_element_never_a_rule() {
        let c = add_cap_element("203.0.113.7".parse().unwrap(), Proto::Tcp, 8443);
        assert_eq!(c.0[0], "add");
        assert_eq!(c.0[1], "element");
        assert!(c.0.contains(&"cap_pinned".to_string()));
        ...
        let d = del_cap_element("198.51.100.9".parse().unwrap(), Proto::Udp, 51820);
        assert_eq!(d.0[0], "delete");
        assert!(d.0.join(" ").contains("{ 198.51.100.9 . udp . 51820 }"));
    }

    #[test]
    fn parse_cap_set_elements_splits_on_space_dot_space_not_ip_dots() {
        ...
        let got = parse_cap_set_elements(listing);
        ...
        assert!(parse_cap_set_elements("set cap_pinned { type ipv4_addr . inet_proto . inet_service }").is_empty());
    }

    #[test]
    fn apply_cap_reconciles_union_and_rolls_back_on_error() {
        ...
        let present = apply_cap(&mut rec, &desired).unwrap();
        ...
        let e = apply_cap(&mut rec2, &desired).unwrap_err();
    ...
    #[test]
    fn apply_cap_from_empty_adds_only_desired_and_never_a_rule() {
        // The weather-migration shape: two tcp:443 tuples land as concat elements in @cap_pinned.
        let mut rec = Rec::new(); // empty live set
        let a = ("104.16.1.1".parse().unwrap(), Proto::Tcp, 443);
        let b = ("104.16.2.2".parse().unwrap(), Proto::Tcp, 443);
        let got = apply_cap(&mut rec, &[a, b]).unwrap();
        assert_eq!(got.len(), 2);
        let muts = rec.mutations();
        ...
        // reconcile to empty (unbless) deletes exactly the two elements.
        let mut rec2 = Rec::new();
        rec2.live = "elements = { 104.16.1.1 . tcp . 443, 104.16.2.2 . tcp . 443 }".into();
        apply_cap(&mut rec2, &[]).unwrap();
        assert_eq!(rec2.mutations().iter().filter(|c| c.0[0] == "delete").count(), 2);
    }
